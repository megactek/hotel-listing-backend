...
function getHotels(call, callback) {
  const query = call.request.location;
...
function filterHotels(call, callback) {
  ...
  const filteredHotels = hotels.filter((hotel) => {
    ...
    return (
      ...
      hotel.location.toLowerCase().includes(location.location.toLowerCase())
    );
  });
  if (filteredHotels.length) {
    callback(null, {
    ...
    callback(null, { err: "no hotels found" });
  }
}
